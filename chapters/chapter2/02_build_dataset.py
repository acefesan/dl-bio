...
Loads the CAFA3 protein dataset components (sequences, taxonomy, GO terms,
embeddings), merges them with Hierarchical Orthologous Groups (HOGs) from OMA,
and saves the result with provenance metadata.

Usage:
    python 02_build_dataset.py                    # Full dataset
    python 02_build_dataset.py --skip-taxonomy    # Skip UniProt API calls
    python 02_build_dataset.py --fetch-hogs       # Fetch HOGs from OMA (slow)
    python 02_build_dataset.py --hog-cache hogs.csv  # Use cached HOG data
    python 02_build_dataset.py --output-dir /path # Custom output directory
"""
...
SEQUENCES_FILE = DATASETS_PATH / "train_sequences.fasta"
TAXONOMY_FILE = DATASETS_PATH / "train_taxonomy.tsv.zip"
LABELS_FILE = DATASETS_PATH / "train_terms.tsv.zip"
GO_DESCRIPTIONS_FILE = DATASETS_PATH / "go_term_descriptions.csv"
EMBEDDINGS_FILE = DATASETS_PATH / "all_species_embeddings.feather"
...
def fetch_hogs_from_oma(
    ...
    Uses single-protein lookups since the bulk API requires OMA IDs.
...
def build_dataset(
    ...
    if hog_cache_path and hog_cache_path.exists():
        ...
        if hog_cache_path:
            hog_cache_path.parent.mkdir(parents=True, exist_ok=True)
            hog_df.to_csv(hog_cache_path, index=False)
    else:
...
def create_metadata(stats: dict, output_path: Path, hog_included: bool = False) -> dict:
    ...
    metadata = {
        ...
        "generator_script": "chapters/chapter2/02_build_dataset.py",
        "output_file": str(output_path.name),
        "source_datasets": {
            "sequences": {
                "file": str(SEQUENCES_FILE.relative_to(PROJECT_ROOT)),
                "description": "CAFA3 training protein sequences (FASTA format)",
            },
            "taxonomy": {
                "file": str(TAXONOMY_FILE.relative_to(PROJECT_ROOT)),
                "description": "Protein to taxonomy ID mapping",
            },
            "labels": {
                "file": str(LABELS_FILE.relative_to(PROJECT_ROOT)),
                "description": "Protein GO term annotations (BPO, MFO, CCO)",
            },
            "go_descriptions": {
                "file": str(GO_DESCRIPTIONS_FILE.relative_to(PROJECT_ROOT)),
                "description": "GO term ID to description mapping",
            },
            "embeddings": {
                "file": str(EMBEDDINGS_FILE.relative_to(PROJECT_ROOT)),
        ...
        "columns": {
            ...
            "ME:1-ME:N": "ESM2 embedding dimensions",
        },
    }
...
def main():
    ...
    parser.add_argument("--fetch-hogs", action="store_true",
                        help="Fetch HOGs from OMA browser API (slow, ~24h for full dataset)")
    parser.add_argument("--hog-cache", type=str, default=None,
    ...
    if args.output_dir:
        ...
        output_dir = DATASETS_PATH / "cafa3_merged"
