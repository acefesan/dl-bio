...
Usage:
    python compute_embeddings.py --limit 100  # Test run with 100 sequences
    python compute_embeddings.py              # Full run (all sequences)
    python compute_embeddings.py --resume     # Resume from partial results
    python compute_embeddings.py --max-seq-length 4000  # Skip very long sequences
"""
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="assets/proteins/datasets/all_species_embeddings.feather",
    ...
    parser.add_argument(
        ...
        type=str,
        default="checkpoints",
        help="Directory to save batch checkpoints (default: checkpoints)"
    )
    parser.add_argument(
        "--max-seq-length",
        type=int,
        default=None,
        help="Skip sequences longer than this (to avoid OOM on very long sequences)"
    )
    args = parser.parse_args()

    # Paths
    base_path = Path(__file__).parent
    fasta_path = base_path / "assets/proteins/datasets/train_sequences.fasta"
    output_path = base_path / args.output
    checkpoint_dir = base_path / args.checkpoint_dir
