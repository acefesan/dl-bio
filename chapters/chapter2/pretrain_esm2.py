...
Usage:
    ...
    python chapters/chapter2/pretrain_esm2.py --dataset-slice --num-steps 50 --batch-size 4

    # Toy run with defaults (10k sequences, 5000 steps)
    python chapters/chapter2/pretrain_esm2.py --dataset-slice

    # Full dataset (requires significant compute)
    python chapters/chapter2/pretrain_esm2.py
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--save-total-limit", type=int, default=3,
                        help="Max checkpoints to keep (default: 3, -1 for unlimited)")
...
def main():
    ...
    if args.dataset_slice:
        print(f"\nLoading dataset: {DATASET_NAME} (streaming first {SLICE_SIZE:,} sequences)")
        stream = load_dataset(DATASET_NAME, split="train", streaming=True)
        rows = list(stream.take(SLICE_SIZE))
        from datasets import Dataset
        raw_dataset = Dataset.from_list(rows)
    else:
        print(f"\nLoading dataset: {DATASET_NAME} (full)")
