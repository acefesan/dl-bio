...
Whole-Census A1 panel expression (built by `census_a1_panel.py` on 2026-09-22), from the local CELLxGENE Census 2025-11-08 SOMA at `/mnt/bulk/dl_bio/`:
- `figures/a1_census_human_by_cell_type.csv` + `..._by_tissue.csv` — exact per-gene stats over **all 96,591,226 primary human cells**, 870 cell types, no subsampling and no scoring.
- `figures/a1_census_{macaque,marmoset,chimp}_by_cell_type.csv` — same for the three non-human primates (2.93M / 1.71M / 158K cells).
- `figures/a1_census_spatial_{human,mouse}_by_tissue.csv` — Visium/Slide-seq spot data.

Dissociated mouse was absent from the local sync and is being re-added; spatial collections contain no brain tissue.
...
**Whole-Census view (96.6M primary human cells, 870 cell types) reframes ADORA1 as a deep-layer cortical gene.** Computed exactly over every cell rather than by scoring a subsample. Top ADORA1 cell types by percent-expressing are near-projecting glutamatergic cortical neurons (66.9%), L5 extratelencephalic (63.5%), L5/6 near-projecting (62.2%) and corticothalamic-projecting (57.1%); brain leads all tissues at 18.7% across 29M brain cells, with spinal cord (12.2%) and eye (4.7%) far behind. Two things complicate the existing narrative. First, **both medium spiny neuron classes carry ADORA1 at ~54-55%**, so the tidy "A1 in cortex/hippocampus, A2A in striatum" split is cleaner in the telling than in the data. Second, **hippocampal pyramidal neurons rank only 10th** — but this is a granularity artifact rather than a contradiction, because the Census `cell_type` axis has no CA4 term and collapses all CA subfields into one label, while HBCA's `supercluster_term` resolves CA4 separately. HBCA cells are themselves inside this 96.6M, so the two analyses are overlapping, not independent. The most consequential result is that **the downstream machinery is near-universal where ADORA1 is present** (GIRK, Gi/o and both presynaptic Ca²⁺ channels at 85-99% of cells) while ADORA1 itself sits at 67-76% — the receptor, not the pathway, is the variable component, which is the strongest argument yet for reporting A1 gene-by-gene instead of as a panel score. The same deep-layer cortical pattern holds in chimp (66.6%), macaque (75.8%) and marmoset (68.4%), spanning ~40M years of primate divergence, though all three non-human censuses are cortex-focused so the *prevalence* similarity is more defensible than the ranking.
